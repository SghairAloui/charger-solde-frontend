...
export class OrdersComponent implements OnInit, OnDestroy {
  ...
  loading = false;
statusFilter: RechargeStatus | 'REJECTED_GROUP' | '' = '';
  searchTerm = '';
  copiedPhone = false;
  currentPage = 0;
  totalPages = 0;
  ...
  cancelMessage = '';
  selectedOrderToCancel!: RechargeRequest;
  processing = false;
...
setFilter(status: RechargeStatus | 'REJECTED_GROUP' | ''): void {
...
loadOrders(): void {
  ...
  if (this.statusFilter === 'REJECTED_GROUP') {
    obs = this.adminService.getRechargeRequests(this.currentPage);
  } 
  else if (this.statusFilter) {
    obs = this.adminService.getRechargesByStatus(
  ...
  obs.pipe(takeUntil(this.destroy$)).subscribe({
    next: (res) => {
      ...
      // 🔥 GROUP FILTER
      if (this.statusFilter === 'REJECTED_GROUP') {
        orders = orders.filter((o: RechargeRequest) =>
          o.status === RechargeStatus.REJECTED ||
          o.status === RechargeStatus.ADMIN_CANCELLED
        );
      }
  ...
  openCancelModal(order: RechargeRequest): void {
  ...
  confirmCancel(): void {
    ...
    this.adminService.cancelRecharge(
      this.selectedOrderToCancel.id,
      this.cancelMessage
    ).subscribe({
      next: () => {
        this.message.success('Commande annulée avec succès');
